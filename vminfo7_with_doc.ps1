<#
.SYNOPSIS
    Comprehensive VM Monitoring Script for Windows

.DESCRIPTION
    This script collects detailed system and performance metrics from a Windows virtual machine.
    It captures information related to CPU, memory, disk, network, services, and processes.
    Ideal for health checks, audits, and performance diagnostics.

.FUNCTIONALITY
    - Collects system hardware/software specs
    - Monitors CPU, memory, and virtual memory
    - Identifies I/O wait and queue lengths
    - Detects high memory consumption (potential leaks)
    - Analyzes thread usage and process handles
    - Evaluates network utilization per interface
    - Designed for scheduled execution every 10s for 2 hours

.OUTPUT
    - Logs saved in C:\VM_Information_Report.txt or CSV (if adapted)
    - Each run logs a snapshot of current state

.EXAMPLE
    Run once manually:
        powershell.exe -File .\vminfo7.ps1

    Schedule for periodic logging:
        Register-ScheduledTask -TaskName "VM_Monitoring_Task" ...
